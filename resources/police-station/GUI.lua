...
function Menu.Option(option, select)
	...
	if not select then
		...
		if(currentOption <= GUI.maxVisOptions and optionCount <= GUI.maxVisOptions) then
			GUI.Text(option, {68, 138, 203, 255, 6}, {menuX, optionCount * 0.035 + 0.125},  {0.5, 0.5 }, true)
			GUI.Rect(GUI.optionRect, { menuX, optionCount * 0.035 + 0.1415 }, { 0.23, 0.035 })
			if(thisOption) then
				GUI.Text(option, {255, 255, 255, 255, 6}, {menuX, optionCount * 0.035 + 0.125},  {0.5, 0.5 }, true)
				GUI.Rect({68, 138, 203, 255}, { menuX, optionCount * 0.035 + 0.1415 }, { 0.23, 0.035 })
			end
		elseif (optionCount > currentOption - GUI.maxVisOptions and optionCount <= currentOption) then
			GUI.Text(option, {68, 138, 203, 255, 6}, { menuX, optionCount - (currentOption - GUI.maxVisOptions) * 0.035 + 0.125 }, { 0.5, 0.5 }, true);
			GUI.Rect(GUI.optionRect, { menuX, optionCount - (currentOption - GUI.maxVisOptions) * 0.035+0.1415 }, { 0.23, 0.035 });
			if(thisOption) then
				GUI.Rect(GUI.scroller, { menuX, optionCount - (currentOption - maxVisOptions) * 0.035 + 0.1415 }, { 0.23, 0.035 })
			end
...
function Menu.OptionData(option, text)
	...
	if(currentOption <= GUI.maxVisOptions and optionCount <= GUI.maxVisOptions) then
		GUI.Text(RightAlign(text), GUI.optionText, { menuX + 0.06, optionCount * 0.035 + 0.125 }, { 0.5, 0.5 }, false)
	elseif(optionCount > currentOption - GUI.maxVisOptions and optionCount <= currentOption) then
		GUI.Text(RightAlign(text), GUI.optionText, { menuX + 0.06, optionCount - (currentOption - GUI.maxVisOptions) * 0.035 + 0.125 }, { 0.5, 0.5 }, false)
	end
...
function Menu.Bool(option, bool, cb)
	...
	if(currentOption <= GUI.maxVisOptions and optionCount <= GUI.maxVisOptions) then
		if(bool) then
			GUI.Text("~r~Return", GUI.optionText, { menuX + 0.085, optionCount * 0.035 + 0.125 }, { 0.5, 0.5 }, true)
		else
			GUI.Text("~g~Sign out", GUI.optionText, { menuX + 0.085, optionCount * 0.035 + 0.125 }, { 0.5, 0.5 }, true)
		end
	elseif(optionCount > currentOption - GUI.maxVisOptions and optionCount <= currentOption) then
		if(bool) then
			GUI.Text("~r~Return", GUI.optionText, { menuX + 0.085, optionCount - (currentOption - GUI.maxVisOptions) * 0.035 + 0.125 }, { 0.5, 0.5 }, true)
		else
			GUI.Text("~g~Sign out", GUI.optionText, { menuX + 0.085, optionCount - (currentOption - GUI.maxVisOptions) * 0.035 + 0.125 }, { 0.5, 0.5 }, true)
		end
	end
