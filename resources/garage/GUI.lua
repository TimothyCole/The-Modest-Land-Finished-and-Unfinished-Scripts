...
function Menu.Option(option, select)
	...
	if not select then
		...
		elseif (optionCount > currentOption - GUI.maxVisOptions and optionCount <= currentOption) then
			...
			GUI.Rect(GUI.optionRect, { menuX, optionCount - (currentOption - GUI.maxVisOptions) * 0.035 + 0.125  }, { 0.23, 0.035 });
			if(thisOption) then
				GUI.Rect(GUI.scroller, { menuX, optionCount - (currentOption - GUI.maxVisOptions) * 0.035 + 0.125 }, { 0.23, 0.035 })
			end
		end
	else
		if(currentOption <= GUI.maxVisOptions and optionCount <= GUI.maxVisOptions) then
...
function Menu.OptionData(option, text)
	...
	if(currentOption <= GUI.maxVisOptions and optionCount <= GUI.maxVisOptions) then
		GUI.Text(text, GUI.optionText, { menuX + 0.06, optionCount * 0.035 + 0.125 }, { 0.5, 0.5 }, false)
	elseif(optionCount > currentOption - GUI.maxVisOptions and optionCount <= currentOption) then
		GUI.Text(text, GUI.optionText, { menuX + 0.06, optionCount - (currentOption - GUI.maxVisOptions) * 0.035 + 0.125 }, { 0.5, 0.5 }, false)
	end
